...
import { resolveStarshipPath, isStarshipInstalled } from '../../src/main/terminal/starship-binary';
...
function expectedBundledName(): string {
  ...
  return `starship-${platformName}-${arch}${platform === 'win32' ? '.exe' : ''}`;
}

describe('terminal/starship-binary - resolveStarshipPath', () => {
  ...
  it('优先返回 packaged 内置二进制（生产模式）', () => {
    ...
    const result = resolveStarshipPath();
  ...
  it('内置二进制都不存在时回退到系统 PATH（开发模式）', () => {
    (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath = '/fake/electron/resources';
    mockExistsSync.mockReturnValue(false);
    mockExecFileSync.mockReturnValue('/usr/local/bin/starship\n');

    const result = resolveStarshipPath();
    expect(result).toBe('/usr/local/bin/starship');
  });
  ...
  it('全部不可用时返回 null', () => {
    ...
    expect(resolveStarshipPath()).toBeNull();
  });
  ...
  it('生成的二进制基础名包含 starship 前缀和平台后缀', () => {
    // 间接验证：通过 packaged 模式的返回路径检查命名约定
    const fakeResources = '/fake/electron/resources';
    (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath = fakeResources;
    ...
    const result = resolveStarshipPath();
    expect(result).toBeTruthy();
    expect(result).toContain('starship');
    // 应包含平台标识（win / mac / linux）和架构标识（x64 / arm64）
    const platform = process.platform;
    const platformName = platform === 'win32' ? 'win' : platform === 'darwin' ? 'mac' : 'linux';
    expect(result).toContain(platformName);
    expect(result).toContain(process.arch);
  });
});

describe('terminal/starship-binary - isStarshipInstalled', () => {
  let originalResourcesPath: string | undefined;
