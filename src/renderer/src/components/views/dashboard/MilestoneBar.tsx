...
export type MilestoneStep = {
  ...
  onClick?: () => void;
  /** 节点附带的子动作按钮（如「后仿验证」的后仿用例调试 / 时序用例分析） */
        ...
        <div key={step.label} className="flex items-center">
          ...
          >
            <button
              ...
              aria-label={clickable ? `打开${step.label}` : undefined}
              className={cn(
                ...
                clickable && 'cursor-pointer hover:border-primary hover:bg-primary/20 hover:ring-2 hover:ring-primary/20',
            ...
            <div className="ml-1 flex shrink-0 flex-col gap-0.5">
              {step.actions.map((action) => (
                <button
                  ...
                  className="rounded border border-border/60 px-1.5 py-0.5 text-[10px] text-muted-foreground transition-colors hover:border-primary/50 hover:text-primary"
