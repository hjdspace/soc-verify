...
| 7. 插件生态管理 | 进行中 | 已完成 `~/.socverify/plugins` 用户目录发现与覆盖优先级；安装、升级、卸载和插件市场入口待实现 |
...
- 扩展宿主 API 通过 `PluginActivationContext` 暴露，iframe 视图不能直接访问 Electron API。后端插件当前属于可信本地代码，仍在 Electron 主进程中运行。
